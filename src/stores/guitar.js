...
export const useGuitarStore = defineStore('guitar', () => {
    ...
    const createGuitar = async ({name, fullName, category, color, description, specifications, care_maintenance, price, rating, discount, stock, image_url}) => {
        spinner.value = true;
        try {
            const response = await axios.post('https://clownfish-app-dwejp.ondigitalocean.app/api/admin/guitars/store', {
                name: name,
                full_name: fullName, 
                category: category,
                color: color,
    ...
    const indexGuitar = async() => {
        spinner.value = true;
        try {
            const response = await axios.get('https://clownfish-app-dwejp.ondigitalocean.app/api/guitars');
    ...
    const showGuitar = async(id) => {
        try {
            ...
            const response = await axios.get(`https://clownfish-app-dwejp.ondigitalocean.app/api/guitar/${id}`);
    ...
    const UpdateGuitar = async ({name, fullName, category, color, description, specifications, care_maintenance, price, rating, discount, stock, image_url}, id) => {
        try {
            const response = await axios.put(`https://clownfish-app-dwejp.ondigitalocean.app/api/admin/guitars/update/${id}`, {
                name: name,
                full_name: fullName, 
                category: category,
                color: color,
    ...
    const deleteGuitar = async (id) => {
        spinner.value = true;
        try {
            const response = await axios.delete(`https://clownfish-app-dwejp.ondigitalocean.app/api/admin/guitars/destroy/${id}`, {
                headers: {
                    'Authorization': `Bearer ${accesToken.value}`
                }
            });
