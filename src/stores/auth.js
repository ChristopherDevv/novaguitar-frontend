...
export const useAuthStore = defineStore('auth', () => {
    ...
    const login = async ({email, password}) => {
        spinner.value = true;
       try {
        const response = await axios.post('https://clownfish-app-dwejp.ondigitalocean.app/api/user/login', {
            email: email,
            password: password
        });
    ...
    const register = async ({firstName, lastName, email, password}) => {
        spinner.value = true;
        try {
            const response = await axios.post('https://clownfish-app-dwejp.ondigitalocean.app/api/user/register', {
    ...
    const logout = async () => {
        spinner.value = true; 
        try {
            const response = axios.get('https://clownfish-app-dwejp.ondigitalocean.app/api/user/logout', {
                headers: {
    ...
    const userUpdate = async ({firstName, lastName, email, password}) => {
        spinner.value = true;    
        try {
            const response = await axios.put(`https://clownfish-app-dwejp.ondigitalocean.app/api/user/update/${authUser.value.user.id}`,{
                first_name: firstName,
                last_name: lastName,
                email: email,
                is_admin: true,
    ...
    const userDelete = async () => {
        spinner.value = true; 
       try {
        const response = await axios.delete(`https://clownfish-app-dwejp.ondigitalocean.app/api/user/destroy/${authUser.value.user.id}`, {
            headers: {
                'Authorization': `Bearer ${authUser.value.access_token}`
            }
        });
