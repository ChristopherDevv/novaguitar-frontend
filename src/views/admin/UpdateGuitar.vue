...
onMounted( async() => {
    ...
    name.value.value = dataGuitar.value.name;
    fullName.value.value = dataGuitar.value.full_name;
    category.value.value = dataGuitar.value.category;
    color.value.value = dataGuitar.value.color;
    description.value.value = dataGuitar.value.description;
    specifications.value.value = dataGuitar.value.specifications;
    care_maintenance.value.value = dataGuitar.value.care_maintenance;
    price.value.value = dataGuitar.value.price;
    rating.value.value = dataGuitar.value.rating;
    discount.value.value = dataGuitar.value.discount;
    stock.value.value = dataGuitar.value.stock;
    image_url.value.value = dataGuitar.value.image_url;
})

const updateItem = handleSubmit((values) => {
    guitar.UpdateGuitar(values, route.params.id);
})
    ...
    <div class="my-28 md:my-36 px-4 md:px-4 lg:px-0 mx-auto md:max-w-4xl lg:max-w-5xl xl:max-w-[1130px]">
         <div class="flex flex-col lg:flex-row items-start justify-between gap-16 lg:gap-0">
             <div class="w-full lg:w-1/3 flex flex-col items-center justify-center lg:items-start lg:justify-start">
                 <img class="w-20 h-auto" src="https://img.icons8.com/3d-fluency/94/open-box.png" alt="open-box"/>
                 <h2 class="mt-1 font-bold text-2xl">Update article for store</h2>
                 ...
                 <div class="w-full md:max-w-xs px-8 py-5 bg-slate-800 rounded-lg mt-5">
                     <RouterLink :to="{name: 'admin-guitar'}" exact-active-class="text-green-500">
                         <div class="flex items-center gap-3">
                             <svg xmlns="http://www.w3.org/2000/svg" class="w-5 fill-current text-green-500" viewBox="0 0 24 24"><path d="M4 13h6a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1zm-1 7a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1v-4a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1v4zm10 0a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1v-7a1 1 0 0 0-1-1h-6a1 1 0 0 0-1 1v7zm1-10h6a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1h-6a1 1 0 0 0-1 1v5a1 1 0 0 0 1 1z"></path></svg>
             ...
             <div class="w-full lg:w-2/3 min-h-[200px]">
                 <div class="w-full bg-slate-800 p-8 rounded-lg"> 
                     <form @submit.prevent="submitTry" class="w-full" novalidate>
                        ...
                        <div class="flex flex-col md:flex-row items-center justify-center w-full mt-5">
                         ...
                         <div class="mt-8">
                             <Modal v-if="meta.valid" :onAccept="updateItem">
                                 <template v-slot:button>
                                     <PrimaryButton type="submit">Update item</PrimaryButton>
                                 </template>
                                 <template #title>Update item</template>
                                 <template #message>Are you sure to update this item?</template>
                             </Modal>
                             <PrimaryButton v-else type="submit">Update item</PrimaryButton>
                         </div>
                         </form>
